...
gulp.task('rename', function ()  {
    var newname;
    var author = 'Michael Schuster';
    var email  = 'development@unltd-networx.de';
    ...
    if (!newname) {
        console.log('Please write the new energymanager name, like: "gulp rename --name mywidgetset" --author "Author Name"');
        process.exit();
    }
    ...
    if (fs.existsSync(__dirname + '/admin/energymanager.png')) {
        fs.renameSync(__dirname + '/admin/energymanager.png',              __dirname + '/admin/' + newname + '.png');
    }
    if (fs.existsSync(__dirname + '/widgets/energymanager.html')) {
        fs.renameSync(__dirname + '/widgets/energymanager.html',           __dirname + '/widgets/' + newname + '.html');
    }
    if (fs.existsSync(__dirname + '/widgets/energymanager/js/energymanager.js')) {
        fs.renameSync(__dirname + '/widgets/energymanager/js/energymanager.js', __dirname + '/widgets/energymanager/js/' + newname + '.js');
    }
    if (fs.existsSync(__dirname + '/widgets/energymanager')) {
        fs.renameSync(__dirname + '/widgets/energymanager',                __dirname + '/widgets/' + newname);
    }
    var patterns = [
        {
            match: /energymanager/g,
            replacement: newname
        },
        {
            match: /energymanager/g,
        ...
        {
            match: /Energymanager/g,
            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'Energymanager'
        },
        {
            match: /Michael Schuster/g,
            replacement: author
        },
        {
            match: /development@unltd-networx.de/g,
            replacement: email
        }
    ];
    var files = [
